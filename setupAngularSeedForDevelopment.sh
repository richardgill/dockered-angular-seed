...
sudo docker run -v $PWD:/mount/angular-webui/ -p 9000:9000 -p 9876:9876 -p 4444:4444 -p 35729:35729 -e IP=$(hostname -I | awk '{print $1}') -i -t angular-webui bash -c "cd /mount/angular-webui/ && ln -s /tmp/node_modules . && ln -s /tmp/bower_components . && scripts/startXServerForFirefox.sh && npm install && bower install --allow-root && scripts/setupE2ETests.sh && grunt serve; bash"
